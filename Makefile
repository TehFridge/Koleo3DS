...
APP_VER				:= 1024
APP_TITLE			:= Koleo3DS
APP_DESCRIPTION		:= Klient Koleo na Nintendo 3DS
APP_AUTHOR			:= TehFridge
PRODUCT_CODE		:= KOLE3DS
UNIQUE_ID			:= 0x3d7a2
...
# Common flags used in both builds
BaseCFLAGS := -Wall -mword-relocations \
...
ifdef DEBUG
    # Debug Flags:
    # -O0: No optimization (critical for debugging variables)
    # -g3 -ggdb: Maximal debug information
    # -DDEBUG: Defines the macro "DEBUG" so you can use #ifdef DEBUG in C code
    CFLAGS := $(BaseCFLAGS) -g3 -ggdb -O0 -fno-omit-frame-pointer -fstack-protector-strong -fno-inline -DDEBUG
else
    # Release Flags:
    # -O2: High optimization (standard for release)
    # -DNDEBUG: Standard macro to disable asserts
    CFLAGS := $(BaseCFLAGS) -O2 -DNDEBUG
endif

# Append libraries and includes to the final CFLAGS
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(GFXBUILD) $(TARGET).3dsx $(OUTPUT).smdh $(TARGET).elf $(TARGET).cia 
